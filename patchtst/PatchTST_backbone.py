...
class PatchTST_backbone(nn.Module):
    ...
    def forward(self, z):                                                                   # z: [bs x nvars x seq_len]
        # norm
        if self.revin: 
            # z = z.permute(0,2,1) # commented
            z = self.revin_layer(z, 'norm')
            # z = z.unsqueeze(2) # commented
            z = z.reshape(z.size(0), 64, 10) # added. size of 64 , 10 are hardcoded. Change with different datasets.
...
class Flatten_Head(nn.Module):
    def __init__(self, individual, n_vars, nf, target_window, head_dropout=0):
        ...
        self.individual = individual
        self.n_vars = n_vars
        print('indvidual', self.individual)
        print('n_vars', self.n_vars)
